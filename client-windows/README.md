...
**2. 复制 [`config.example.yaml`](config.example.yaml) 成 `config.yaml`，放在 `agent.exe` 旁边**，
...
| `internal/config/` | YAML 加载与「配错就不启动」的校验 |
...
> [!IMPORTANT]
> CI 在 Linux 上只能用 `GOOS=windows` 交叉编译 + `go vet` 这个模块，**跑不了它的单测**。
> 所以改完请务必在 Windows 本机跑一遍 `go test ./...`，这是这部分代码唯一的测试关口。
